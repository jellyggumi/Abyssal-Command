...
|---|---|---|---|
| `design/core-loop.md` numeric G7 model | absent | `cinder-extraction-circuit`, 45 s intended, 30–180 s allowed, at least 3 of 4 authored actions, at least 1 `ELITE_EXTRACTED` reward | Restores the signed G7 contract from `production/gate-reviews/stage-2-g7.md`; it is a measurement target, not a claim that the current UI teaches it. |
| `defense-catalog.js#STAGES[cinder-span].gateTicks` | `720` ticks | `900` ticks, per `balance-sheet.md` | Adds bounded pre-elite pressure; post-retune Cinder duration must remain inside the 30–180 s loop band. |
...
QA reports `voluntary reentries / eligible re-entry decisions`; G7 needs **≥14/20 (70%)**. Recompute each circuit duration from first actionable movement to result/staging return and require every completed circuit to lie in the inclusive **30–180 s** band. Each observer row must also retain at least **3 unique** authored player-action IDs and at least **1** reward-event ID (`ELITE_EXTRACTED`). A lower repeat rate, out-of-band duration, insufficient action/reward evidence, missing evidence step, or player-visible failure leaves G7 **BLOCKED**.
