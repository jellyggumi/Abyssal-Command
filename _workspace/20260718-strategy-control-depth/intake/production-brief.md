...
| G3 | game-qa owns archetype rotation; game-designer owns corrective design | ≥5 archetypes tested; ≥3 independently viable with distinct strategies and 45–55% band; no archetype >50% optimal-play dominance | Fewer than three viable strategies is `REDO` to Stage 2b. |
| G6 | game-programmer measures; game-qa independently verifies; director adjudicates | input p95 ≤100 ms/p99 ≤150 ms; p95 frame ≤16.7 ms; long frames <0.5%; 30-min memory growth ≤5%; 30/60/120 Hz motion/camera bands; 60-case renderer/input matrix with 0 semantic/save divergence; 1,000×150 deterministic fuzz with 0 mismatch; rollback/readiness/telemetry 100% complete | Stage 3 cannot pass on a source audit or unit test alone. |
...
**G5 carry guard:** no monetization or paid-power surface changes in this beat. The previous “no revenue surface” boundary may carry only as an unchanged constraint. A commercial or reward-access change reopens full G5 before Stage 2 exit.
...
| Main’s isolated `/tmp` Echo `8→6` copied-module probe | **Candidate rationale only** | shows one-family direction worth measuring: casual 51.5%/200, Echo defeats 49, fuzz 150,000/0 | no durable command/build artifact; omitted integrated caps/topology/control and retained polarized strategies, so it cannot pass G2/G3 or v7 determinism |

**Current evidence verdict:** all current-version gate values remain `FIX` or `NOT RUN`. The v6 packet is now a pre-change comparator: its 40% casual result and 0% rusher/comeback explain the selected Echo/cap direction, while its 1.1685× 12-pair combo ratio and 150,000-operation/6,051-round-trip/zero-finding double run cannot pass v7. G2 lacks a v7 mechanic audit, TTK, matchup, and all-stage combo matrix; G3 lacks v7 archetype rotation; G6 lacks v7 renderer/input/latency/frame/path/soak/save evidence and explicit v6-envelope rejection; G7 lacks v7 timing and voluntary re-entry; G8 retains only its 0/6 bounded frequency subcriterion. The newest resource-refinement run also ended `FIX` for its scoped Stage 1 exit because direct failure equivalence, compact/reduced-motion, and other evidence remained incomplete (`_workspace/20260718-resource-refinement/qa/gate-measurements.md:18-27`).
