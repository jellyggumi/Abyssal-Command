...
- **decision:** remain in Stage 2 for balance, tactical-map, core-loop, novelty, parity, and control implementation. Stage 2 exit and Stage 3 evidence execution remain blocked until the director records G2/G3/G7/G8 `PASS`, the G5 no-commercial-change guard is confirmed, and open S1 defects equal zero.
...
- **consequence:** G5 is a no-change carry guard. Any commercial/reward-access proposal reopens full G5. Any multiplayer proposal becomes a separate architecture/intake cycle and cannot justify a duplicate reducer here.
...
- **gate adjudication now:** **G2 `FIX`** (v6 40% is pre-change risk only; integrated v7 balance/TTK/combo evidence absent); **G3 `FIX`** (v7 five-archetype rotation and three independently viable strategies absent); **G6 `FIX/NOT RUN`** (specification accepted, v7 browser/runtime/save evidence absent); **G7 `FIX`** (numeric model accepted, v7 timing/re-entry/human evidence absent); **G8 `FIX`** (0/6 bounded frequency subcriterion passes, QA impression/comprehension absent). Stage 2 implementation may continue; Stage 2 has not exited and Stage 3 has not passed.
...
- **revisit triggers:** any route outside its structural/outcome bands, casual outside 45–55%, persistent polarized archetypes, renderer/input/save divergence, feedback or frame/path/soak miss, Undertow comprehension below 90% or impression below 4/5, or any proposed reward source-value movement.
