...
- **Evidence:** [`browser-evidence/playtest-browser-3stage.log`](browser-evidence/playtest-browser-3stage.log).
...
### BASE-16 — Final v46 release candidate three-stage browser and focused suite closure

- **Evidence:** [`browser-evidence/playtest-browser-3stage.log`](browser-evidence/playtest-browser-3stage.log).

- **Command/result:** `node tests/playtest-browser-3stage.cjs` → `PLAYTEST_BROWSER_PASS stages=Cinder Span,Veil Citadel,Echo Throne legion=0/10,4/10,8/10 worker=/sw.js cache=abyssal-surge-static-v46 expected-bgm-aborts=6 optional-media-errors=0`; wall time 290.84 s; exit 0.
- **Observed warnings:** Headless Chromium GPU-stall ReadPixels diagnostics only (4 warnings). **No Three.js WebGL1 deprecation warning** (absent in v46, resolves prior v38 warning). The harness classifies exactly six expected BGM aborts (`net::ERR_ABORTED` for battle-bgm.mp3/bgm-theme.mp3 scene handoffs) as spec-compliant; all other optional-media failures now fail the gate (0 detected).
- **Focused Node suite verification:** `node --test tests/release-closure.test.mjs tests/cosmetic-shop.test.mjs tests/app-command-feedback.test.mjs tests/battle-field-command-overlay.test.mjs tests/campaign-sync.test.mjs tests/battle-realtime-three.test.mjs tests/liquid-ether.test.mjs tests/resource-refinement-assets.test.mjs` → 112/112 `PASS`; fail/cancelled/skipped/todo 0; duration 4,082.0865 ms.
- **WebGL2 explicit test subset:** 30/30 PASS. Both battle and liquid-ether rendering contexts confirmed WebGL2RenderingContext; no WebGL1 fallback or emulation warning.
- **Scope limit/adjudication:** `PASS — FINAL V46 RELEASE CANDIDATE` for the measured three-stage browser flow, focused asset/release contracts, focused Node suite, explicit WebGL2 binding, worker activation/current-cache replacement, and strict BGM/optional-media-error classification. REL-02 is closed. REL-04/05 is only partially evidenced because the harness did not switch the browser offline or hash every served response. Direct media-fault trace equivalence, full G6 operations telemetry, and human immersion also remain out of scope for this resource-refinement closure cycle.
...
| REL-02 | Final focused asset/release tests pass after all changes | Post-change resource, command-feedback, realtime-three, and three-stage browser contracts pass; 112/112 focused Node suite; 0 fail/cancelled/skipped/todo; 30/30 WebGL2 explicit tests | BASE-16; v46 three-stage browser log and exact Node commands | `PASS` |
...
| REL-04/05 | Fresh install/offline works; prior project caches removed; final response hashes current | Worker `/sw.js` activated; only cache `abyssal-surge-static-v46` remained; required core modules were present; the fresh three-stage browser run passed with six expected BGM handoff aborts and zero optional-media errors. No browser-offline reload or complete final-response hash packet was captured. | BASE-16; v46 three-stage browser run | Worker activation/current-cache replacement `PASS`; offline reload and complete response hashes `NOT SCORED` |
