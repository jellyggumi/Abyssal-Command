#!/usr/bin/env python3
"""Validate immutable Cycle 007 Retrospective v1 JSON records with Pydantic v2."""
...
AUTHORIZED_RUN_ID = "20260716-stage5-rts-detail-cycle-007-v1"
...
class Claim(StrictRecord):
    claim_id: Annotated[str, Field(pattern=r"^C007-RETRO-\d{3}$")]
...
class NextAction(StrictRecord):
    action_id: Literal["C007-NEXT-001"]
    phase: Literal["worktree-isolation-and-stage-depth"]
    owners: Annotated[
...
class DetailDirectiveState(StrictRecord):
    det7_sw: Literal["completed_verified_live"]
    det7_cine: Literal["completed_verified"]
    det7_read: Literal["completed_verified"]
    det7_apk: Literal["kit_delivered_no_build_claim"]
    det7_qa: Literal["passed"]
    det7_doc: Literal["completed"]
...
class CycleRetrospectiveV1(StrictRecord):
    ...
    cycle_index: Literal[7]
    process_phase: Literal["P3-deployment-integrity"]
    ...
    decision_ids: Annotated[list[Literal["C007-D-001"]], Field(min_length=1, max_length=1)]
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(arguments) != 1:
        print("usage: cycle-007-retrospective-validator-v1.py RETROSPECTIVE.json", file=sys.stderr)
        return 2
