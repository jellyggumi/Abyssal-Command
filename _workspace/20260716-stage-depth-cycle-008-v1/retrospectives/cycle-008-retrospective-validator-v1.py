#!/usr/bin/env python3
"""Validate immutable Cycle 008 Retrospective v1 JSON records with Pydantic v2."""
...
AUTHORIZED_RUN_ID = "20260716-stage-depth-cycle-008-v1"
...
class Claim(StrictRecord):
    claim_id: Annotated[str, Field(pattern=r"^C008-RETRO-\d{3}$")]
...
class NextAction(StrictRecord):
    action_id: Literal["C008-NEXT-001"]
    phase: Literal["stages-1-4-economy-review-and-apk-build"]
    owners: Annotated[
...
class DetailDirectiveState(StrictRecord):
    det8_gate: Literal["passed_with_main_fix"]
    det8_e2e: Literal["contracts_proven_in_browser"]
    det8_depth: Literal["completed_verified"]
    det8_qa: Literal["passed_live_verified"]
    det8_doc: Literal["completed"]
...
class CycleRetrospectiveV1(StrictRecord):
    ...
    cycle_index: Literal[8]
    process_phase: Literal["P3-stage-depth-convergence"]
    ...
    decision_ids: Annotated[list[Literal["C008-D-001"]], Field(min_length=1, max_length=1)]
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(arguments) != 1:
        print("usage: cycle-008-retrospective-validator-v1.py RETROSPECTIVE.json", file=sys.stderr)
        return 2
