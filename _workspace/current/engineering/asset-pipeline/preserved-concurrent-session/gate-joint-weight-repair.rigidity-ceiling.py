...
Four gates per asset, all measured from the GLB payload without Blender:
  ...
  rigidity single-influence vertices must stay <= min(asset baseline,
           25% of measured vertices). A vertex driven by one bone is rigid; trading
           a seam for a rigid patch is not a fix.
...
MAX_SPREAD = 2
# Rigid (single-influence) vertices are allowed both by asset history and by an
# absolute production-safe ceiling, to guard against pathological all-single-influence
# repairs that pass only because a bad baseline did.
#
# Standard, stated independently of any asset's numbers: a vertex driven by one bone
# cannot bend, so rigid vertices are a skinning defect -- but leaf bones (toe, hand)
# legitimately own a few tip vertices with no bending partner, so the honest bar is a
# small share of the mesh, not zero. 25% is a conservative upper bound that catches
# obviously-bad repairs while still allowing partial-legacy assets to improve.
RIGIDITY_CEILING_FRACTION = 0.25
# A pure "not worse than baseline" rigidity bar is unusable where the asset ships with
# only a handful of rigid vertices: lantern-reaver has inf1=3, so it=20 (inf1=12) would
# be rejected even though it cuts seam edges 285 -> 202. 25% is a coarse fixed budget
# measured in absolute vertex count below.
...
def verdict(before: dict[str, Any], after: dict[str, Any]) -> dict[str, Any]:
    ...
    # Keep a hard cap for pathological all-rigid repairs while also avoiding false
    # negatives when an asset ships with a small baseline count.
    # Candidate single-influence vertices must not rise and must also stay below the
    # conservative absolute ceiling.
    vertices = max(1, before["verticesMeasured"])
    rigidity_ceiling_vertices = int(vertices * RIGIDITY_CEILING_FRACTION)
    rigidity_budget = min(before["singleInfluenceVertices"], rigidity_ceiling_vertices)
