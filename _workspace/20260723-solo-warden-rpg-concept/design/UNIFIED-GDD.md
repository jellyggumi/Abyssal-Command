...
| 포대(Turret) | 후방 300유닛 | 300유닛(밀집) | 재배치 지연 최소, **동료 완전 무피해**(FRONT 0 → 적 타겟팅 대상에서 완전 제외, `qa/review-checkpoint-1-20260725.md` §1.3 실측 확인). Boss Rally Window(§4.3)·후열 시너지(`BACK_ROW_SYNERGY_DAMAGE_BONUS`) 양쪽 다 FRONT≥1 요구라 미적용 — 화력은 3스탠스 중 최저(2026-07-25 정정: 이전 "지속 화력 최대" 서술은 실측과 불일치해 정정됨) | 0 |
