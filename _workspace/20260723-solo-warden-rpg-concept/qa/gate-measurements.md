...
- **R1 (Warden raw-stat ceiling ≤20% of stage power budget)**: Cycle 1's isolated companion-free stress-test is superseded below.

### Cycle 2 — R1 full-protocol measurement (realistic loadouts, all 10 stages)

Computed directly from `qa/evidence-cycle2/sweep-*.json`'s real archetype/seed/stage `statPoints`/`traitIds`/`loadout` via live `deriveWardenRuntimeStats`/`deriveCompanionRuntimeStats` calls (`rpg-catalog.js`) — the literal "stat-only contribution across all 10 stages with realistic loadouts" protocol Cycle 1 could not run. Full detail: `qa/r1-full-protocol-cycle2.md`.

- **Status: FIX.** The stat-only ratio (Warden's normalized damage share vs Warden+companions) genuinely exceeds the 20% ceiling at specific stages for 3 of 7 archetypes: `single-companion-main` (7/10 stages, sunken-bastion→gate-zenith), `rusher` (stages 8-10, a normal 2-3-companion build, not an edge case), `micro-optimizer` (stages 4-7, one RNG branch). 34 of 210 points exceed 20%. Most extreme: `single-companion-main`/`gate-zenith` at 36.84% (1.84× ceiling).
- **Mechanism (structural, not archetype-specific)**: Warden stat investment (`binding-might`: +15 flat damage/point, uncapped growth within the 40-point budget) compounds every stage a player keeps investing, while companion role bonuses are **static fractions** (striker: +0.20/+0.30, fixed regardless of campaign stage). Any archetype that keeps investing in Warden damage stats will see its Warden-share ratio climb over the campaign, independent of loadout size — `rusher`'s 3-companion build hits the same pattern as `single-companion-main`'s 1-companion build, just later (stage 8 vs stage 4).
- **Caveat carried forward**: this is a **stat-only proxy** — equipment tiers (`weapon`/`ward`/`trinket`, up to ×2.00 at T5) aren't recorded in the sweep script's output for either side of the ratio, so this doesn't yet close `UNIFIED-GDD.md` §9.1's literal "measured on final effectiveDamage" binding. Equipment could narrow or widen the gap depending on whether players over-index Warden or companion gear — undetermined by this data. **Recommended next-cycle action**: extend `scripts/run-g2-archetype-rotation.mjs` to record per-stage equipment tier indices, re-run this computation with real `equipment` objects.
- Evidence: `qa/r1-full-protocol-cycle2.md` (full 210-point table), `qa/evidence-cycle2/sweep-*.json`.
