...
if curl -sf --connect-timeout 3 "${BASE_URL}/health" >/dev/null 2>&1; then
  ...
  fail "GET /health — server unreachable at ${BASE_URL}"
  echo "Please start the agentation MCP server: npx agentation-mcp server"
  exit 1
fi
...
BASELINE_COUNT=$(echo "$PENDING" | node -e "const fs = require('fs'); try { console.log(JSON.parse(fs.readFileSync(0, 'utf-8')).count); } catch { console.log(-1); }" 2>/dev/null || echo -1)
...
SESSION_RESP=$(curl -sf -X POST "${BASE_URL}/sessions" -H "Content-Type: application/json" -d '{"url":"http://localhost:8000"}' 2>/dev/null || echo "")
SESSION_ID=$(echo "$SESSION_RESP" | node -e "const fs = require('fs'); try { console.log(JSON.parse(fs.readFileSync(0, 'utf-8')).id || ''); } catch { console.log(''); }" 2>/dev/null || echo "")
...
ANN_RESP=$(curl -sf -X POST "${BASE_URL}/sessions/${SESSION_ID}/annotations" \
  ...
  -d "{\"comment\":\"Test: change button color\",\"element\":\"button\",\"elementPath\":\"body > main > button.cta\",\"x\":50,\"y\":100,\"timestamp\":\"2026-07-19T13:17:04.104Z\"}" 2>/dev/null || echo "")
ANN_ID=$(echo "$ANN_RESP" | node -e "const fs = require('fs'); try { console.log(JSON.parse(fs.readFileSync(0, 'utf-8')).id || ''); } catch { console.log(''); }" 2>/dev/null || echo "")
if [[ -n "$ANN_ID" ]]; then
...
P_COUNT=$(curl -sf "${BASE_URL}/sessions/${SESSION_ID}/pending" 2>/dev/null \
  | node -e "const fs = require('fs'); try { console.log(JSON.parse(fs.readFileSync(0, 'utf-8')).count); } catch { console.log(0); }" 2>/dev/null || echo 0)
...
ACK_STATUS=$(echo "$ACK_RESP" | node -e "const fs = require('fs'); try { console.log(JSON.parse(fs.readFileSync(0, 'utf-8')).status || ''); } catch { console.log(''); }" 2>/dev/null || echo "")
...
RES_STATUS=$(echo "$RES_RESP" | node -e "const fs = require('fs'); try { console.log(JSON.parse(fs.readFileSync(0, 'utf-8')).status || ''); } catch { console.log(''); }" 2>/dev/null || echo "")
...
FINAL_COUNT=$(curl -sf "${BASE_URL}/sessions/${SESSION_ID}/pending" 2>/dev/null \
  | node -e "const fs = require('fs'); try { console.log(JSON.parse(fs.readFileSync(0, 'utf-8')).count); } catch { console.log(-1); }" 2>/dev/null || echo -1)
...
if ! $QUICK; then
  ...
  HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" -X PATCH "${BASE_URL}/annotations/nonexistent-id-12345" \
  ...
  HTTP_CODE=$(curl -s -o /dev/null -w "%{http_code}" "${BASE_URL}/sessions/nonexistent-session-12345" 2>/dev/null || echo "000")
  if [[ "$HTTP_CODE" == "404" || "$HTTP_CODE" == "400" ]]; then
